...
static const CapabilityConfig kConfigs[] = {
    ...
    {{PPC_FEATURE_PSERIES_PERFMON_COMPAT, 0}, "archpmu", &set_pseries_perfmon_compat},
...
static bool HandlePPCLine(const LineResult result,
  ...
  if (GetAttributeKeyValue(line, &key, &value)) {
    if (HasWord(key, "platform")) {
      CopyString(value, strings->platform, value.size + 1);
    } else if (IsEquals(key, str("model"))) {
      CopyString(value, strings->model, value.size + 1);
    } else if (IsEquals(key, str("machine"))) {
      CopyString(value, strings->machine, value.size + 1);
    } else if (IsEquals(key, str("cpu"))) {
      CopyString(value, strings->cpu, value.size + 1);
    }
  }
  return !result.eof;
}
